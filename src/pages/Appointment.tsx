...
type LeaveRecord = {
  ...
  full_day: boolean; half_day_period?: string; time_from?: string; time_to?: string; status: string;
};
...
export default function Appointment() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setDoctors((dbDoctors || []).map(d => ({
        value:      d.doctor_key || d.username,
        label:      d.full_name,
        spec:       d.specialization || '',
        dept:       ['doctor_aravind','doctor_vishali','doctor'].includes(d.role) ? 'Medical Consultation' : 'Specialist Services',
        specialist: d.role,
      })));

      const { data: leaveData } = await supabase
        .from('leave_management').select('*').eq('status', 'active').gte('end_date', today);
      setLeaves(leaveData || []);
    };
    fetchData();
  }, []);
  ...
  const getBlockedInfo = (doctorValue: string, date: string) => {
    ...
    if (fullLeave) {
      return {
        allDay: true, blockedTimes: [] as string[],
    ...
    let fromMins: number, toMins2: number;
    if (hl.time_from && hl.time_to) {
      fromMins = toMins(hl.time_from); toMins2 = toMins(hl.time_to);
    } else if (hl.half_day_period === 'first_half') {
      fromMins = 0; toMins2 = 13 * 60 + 1;
    } else {
    ...
    const blockedTimes = allSlots.filter(slot => { const sm = toMins(slot); return sm >= fromMins && sm < toMins2; });

    const rangeStr = hl.time_from && hl.time_to ? `${hl.time_from} - ${hl.time_to}`
      : hl.half_day_period === 'first_half' ? 'morning (10 AM - 1 PM)' : 'evening (5 PM - 10 PM)';

    return {
      allDay: false, blockedTimes,
      message: hl.specialist === 'clinic_holiday'
        ? `${CLINIC_NAME} is partially closed - unavailable ${rangeStr} on this date.`
        : `${docInfo.label} is unavailable ${rangeStr} on this date.`,
    };
  };
  ...
  const checkLeaveBlock = (doctorValue: string, date: string) => {
    ...
    if (info.allDay) setLeaveBlocked({ blocked: true, message: info.message });
    else if (info.blockedTimes.length > 0) setLeaveBlocked({ blocked: false, message: info.message });
    else setLeaveBlocked({ blocked: false, message: '' });
  };

  const handleChange = (field: string, value: string) => {
    if (field === 'appointment_date') setForm(prev => ({ ...prev, appointment_date: value, appointment_time: '' }));
  ...
  const validateForm = (): string | null => {
    ...
    if (form.patient_email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.patient_email)) return 'Enter a valid email';
    ...
    if (blockedSlots.includes(form.appointment_time)) return 'This time slot is unavailable due to leave. Please choose another time.';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { data: activeLeaves } = await supabase.from('leave_management').select('specialist,start_date,end_date,full_day').eq('status','active').lte('start_date', form.appointment_date).gte('end_date', form.appointment_date);
    if (activeLeaves?.length) {
      const docInfo = doctors.find(d => d.value === form.doctor);
      const blocked = activeLeaves.some(l => l.specialist === 'clinic_holiday' || l.specialist === docInfo?.specialist);
      if (blocked) { setStatus('error'); setErrorMsg('This slot is no longer available due to leave. Please choose another date.'); return; }
    }
    ...
    else { setStatus('success'); toast.success('Appointment request submitted!'); }
  };
          ...
          <div className="bg-white rounded-3xl shadow-card p-6 sm:p-8 md:p-10">
            ...
            <form onSubmit={handleSubmit} className="space-y-6" noValidate>
              ...
              <div>
                ...
                {doctors.length === 0 ? (
                  <div className="flex justify-center py-4"><div className="w-6 h-6 border-2 border-[#0F9FA8]/30 border-t-[#0F9FA8] rounded-full animate-spin"/></div>
                ) : (
              ...
              <div className="grid md:grid-cols-2 gap-5">
                ...
                <div>
                  ...
                  {leaveBlocked.blocked && form.appointment_date ? (
                    ...
                    <p className="text-xs text-gray-400 mt-2">Select a date to see available time slots.</p>
                  ) : (() => {
                    ...
                    return (
                      <div className="space-y-3">
                        ...
                        {groups.map(({ label, slots: groupSlots }) => groupSlots.length > 0 && (
                          <div key={label}>
